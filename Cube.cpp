...
/*Constructeur*/
Cube::Cube() {
    mesh = new Mesh(read_mesh("data/cube.obj"));
    if (!mesh->vertex_count()) return ;
    
    m_ShaderId = read_program("TP/TP4/Shaders/FB_Draw_MRT_Scene.glsl");
    ...
    m_ViewMatLocation = glGetUniformLocation(m_ShaderId, "viewMat");
    m_ProjectionMatLocation = glGetUniformLocation(m_ShaderId, "projectionMat");
